...
def create_dataset(zip_files, output_train_json, output_val_json):
    ...
    # Convert lists back to nested dictionaries
    def convert_to_nested_dict(data):
        nested_dict = {}
        for item in data:
            researcher = item["researcher"]
            review_id = item["review_id"]
            if researcher not in nested_dict:
                nested_dict[researcher] = {}
            nested_dict[researcher][review_id] = {
                "paper": item["paper"],
                "review": item["review"]
            }
        return nested_dict

    train_dataset = convert_to_nested_dict(train_data)
    val_dataset = convert_to_nested_dict(val_data)
