...
def preprocess_function(examples):
    ...
    for paper, review in zip(examples["paper"], examples["review"]):
        # Chunk the paper and review
        paper_chunks = chunk_text(paper, max_length=512, overlap=50)
        review_chunks = chunk_text(review, max_length=512, overlap=50)

        # Ensure that both paper_chunks and review_chunks have the same number of chunks
        num_chunks = max(len(paper_chunks), len(review_chunks))

        # Pad with empty chunks if necessary to match lengths
        paper_chunks += [''] * (num_chunks - len(paper_chunks))
        review_chunks += [''] * (num_chunks - len(review_chunks))

        all_input_chunks.extend(paper_chunks)
        all_target_chunks.extend(review_chunks)

    # Tokenize the chunks
    model_inputs = tokenizer(all_input_chunks, max_length=512, truncation=True, padding="max_length")
    labels = tokenizer(text_target=all_target_chunks, max_length=512, truncation=True, padding="max_length")
...
if __name__ == "__main__":
    ...
    tokenized_datasets = datasets.map(preprocess_function, batched=True, remove_columns=["paper", "review"])
